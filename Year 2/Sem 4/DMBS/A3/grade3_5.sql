...
BEGIN
	...
	SET @StudentID = (SELECT TOP 1 StudentID FROM Students WHERE StudentName = @StudentName ORDER BY StudentID);
	SET @CourseID = (SELECT TOP 1 CourseID FROM Courses WHERE CourseName = @CourseName ORDER BY CourseID);
