...
void testReplace()
{
	...
	ValuePair NullElements = make_pair(NULL_TVALUE, NULL_TVALUE);

	for (int i = 101; i <= 500; ++i)
		assert(m.search(i) == NullElements);

	for (int i = 51; i <= 100; ++i)
	{
		ValuePair OneNullOneNot = make_pair(i, NULL_TVALUE);
		assert(m.search(i) == OneNullOneNot);
	}

	for (int i = 1; i <= 50; ++i)
	{
		ValuePair BothExist = make_pair(i, i);
		assert(m.search(i) == BothExist);
	}
}
