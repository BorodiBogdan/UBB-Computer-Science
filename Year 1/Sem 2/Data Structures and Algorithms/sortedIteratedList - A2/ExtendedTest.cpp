...
void testRemoveBetween(Relation r)
{
	std::cout << "Testing remove between...\n";
	SortedIteratedList testList(r);
	...
	ListIterator right = testList.first();

	for (int i = 1; i <= 100; ++i)
		right.next();

	ListIterator left = testList.first();

	testList.removeBetween(left, right);

	assert(testList.size() == 900);
	...
	ListIterator it = testList.first();
	ListIterator it2 = testList.first();
	it2.next();

	while (it2.valid())
	{
		assert(r(it.getCurrent(), it2.getCurrent()) == 1);
		it.next();
		it2.next();
	}
}
...
void testAllExtended()
{
	...
	testDeleteSearch();
	testQuantity();
	testRemoveBetween(asc);
	testRemoveBetween(desc);
}
