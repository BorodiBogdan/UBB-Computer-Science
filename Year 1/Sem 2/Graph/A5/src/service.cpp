...
bool checkIfColouringIsOk(Graph &g, vector<int> &colorList)
{
    for (auto it = g.getVerticeIterator(); it != g.getEndOfIteration(); ++it)
    {
        int vertex = it->first;

        for (auto it2 = g.getIteratorOutboundEdges(vertex); it2 != g.getEndOfIterationOutboundEdges(vertex); ++it2)
        {
            int newVertex = it2->second;
            if (colorList[vertex] == colorList[newVertex] && vertex != newVertex) {
                return false;
            }
        }
    }
...
void getMinimumColorint(Graph &g, vector<int> &colorList, int &numberOfColors, int k, vector<int> &finalColorList)
{
    // we will perform a backtracking for the colors
    // try eith one color, two colors and so on untile we find a valid coloration
    // when so we will return the colorList

    if (k == g.getNumberOfVertices()) {

        if (checkIfColouringIsOk(g, colorList))
    ...
    {
        colorList[k] = i + 1;
        getMinimumColorint(g, colorList, numberOfColors, k + 1, finalColorList);
        colorList[k] = 0;
    }
}

vector<int> Service::getMinimumColoringGraph()
{
    // Given an undirected graph, find a vertex coloring with minimum number of colors
    // such that no two adjacent vertices have the same color.
    // The graph is represented as an adjacency list.

    vector<int> colorList(this->myGraph.getNumberOfVertices(), 0);
    vector<int> finalColorList(this->myGraph.getNumberOfVertices(), 0);

    for (int i = 1; i <= this->myGraph.getNumberOfVertices(); ++i)
    {
        cout << i << " " << this -> myGraph.getNumberOfVertices() << "\n";
        getMinimumColorint(this->myGraph, colorList, i, 0, finalColorList);

        if (finalColorList[0] != 0)
...
vector<int> Service::getMinimumColouringGraphGreedy() {
    vector<int> colorList(this->myGraph.getNumberOfVertices(), 0);

    for (auto it = this->myGraph.getVerticeIterator(); it != this->myGraph.getEndOfIteration(); ++it) {
        int vertex = it->first;
        vector<bool> usedColors(this->myGraph.getNumberOfVertices() + 1, false);

        for (auto it2 = this->myGraph.getIteratorOutboundEdges(vertex); it2 != this->myGraph.getEndOfIterationOutboundEdges(vertex); ++it2) {
            int newVertex = it2->second;
            usedColors[colorList[newVertex]] = true;
        }
